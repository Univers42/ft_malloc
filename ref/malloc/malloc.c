...
#define ASSERT(p)                                                     \
  ...
  {                                                                   \
    if (!(p))                                                         \
      xbotch((PTR_T)0, ERR_ASSERT_FAILED, CPP_STRING(p), file, line); \
  } while (0)
...
/* Minimum and maxfimum bucket indices for block coalescing. */
...
free_return:; /* Empty statement in case this is the end of the function */
...
#ifdef MALLOC_STATS
int malloc_free_blocks(int size)
{
  int nfree;
  register union mhead *p;
...
void sh_free(PTR_T mem, const char *file, int line)
{
...
void sh_cfree(PTR_T mem, const char *file, int line)
{
